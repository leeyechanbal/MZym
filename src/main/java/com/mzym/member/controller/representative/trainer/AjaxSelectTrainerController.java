...
/**
 * Servlet implementation class AjaxSelectTrainerController
 */
@WebServlet("/selectTrainer.re")
public class AjaxSelectTrainerController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AjaxSelectTrainerController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<Member> mList = new RepreService().selectListTrainer(pi);
		
		HashMap<String, Object> trainerMap = new HashMap<>();
		trainerMap.put("pi", pi);
		trainerMap.put("list", mList);

		response.setContentType("application/json; charset=utf-8");
		new Gson().toJson(trainerMap, response.getWriter());
	}
	...
	private PageInfo  pagingBar(HttpServletRequest request, HttpServletResponse response) {
		// 메뉴바에 있는 일반게시판 메뉴 클릭시 /list.bo?page=1    => 1번 페이지 요청 
				...
				listCount = new RepreService().selectTrainerCount();
