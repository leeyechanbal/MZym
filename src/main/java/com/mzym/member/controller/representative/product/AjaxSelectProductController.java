...
public class AjaxSelectProductController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<Product> pList = new RepreService().selectListProduct(pi);
		
		HashMap<String, Object> productMap = new HashMap<>();
		productMap.put("pi", pi);
		productMap.put("list", pList);
	...
	private PageInfo  pagingBar(HttpServletRequest request, HttpServletResponse response) {
		// 메뉴바에 있는 일반게시판 메뉴 클릭시 /list.bo?page=1    => 1번 페이지 요청 
				...
				listCount = new RepreService().selectProductCount();
